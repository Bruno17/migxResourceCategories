...
$resCategories = array();
$positions = array();
$c2 = $modx->newQuery('mrcResourceCategoryLink');
$c2->where(array('resource_id'=>$resource_id));
if ($resCategoryCollection = $modx->getCollection('mrcResourceCategoryLink',$c2)){
    foreach ($resCategoryCollection as $resCategoryObject){
        $resCategories[] = $resCategoryObject->get('category_id');
        $positions[$resCategoryObject->get('category_id')] = $resCategoryObject->get('pos');
    }
}
